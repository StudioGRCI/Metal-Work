...
begin
  ...
  for v_paso in 1..10 loop
    ...
    when 9 then
      -- Sin aviso a portería, incluso el responsable de entrega debe fallar.
      reset role;
      perform set_config('request.jwt.claim.sub',v_roles->>'JEFE_TALLER',true);
      perform set_config('request.jwt.claims',json_build_object('sub',v_roles->>'JEFE_TALLER','role','authenticated')::text,true);
      set local role authenticated;
      v_negado:=false;
      begin perform public.registrar_salida_fisica(v_acta,'Intento sin aviso a porteria');
      exception when check_violation then v_negado:=true; end;
      if not v_negado then raise exception 'FALLO: salida sin aviso'; end if;
      reset role;
      perform set_config('request.jwt.claim.sub',v_uid::text,true);
      perform set_config('request.jwt.claims',json_build_object('sub',v_uid,'role','authenticated')::text,true);
      set local role authenticated;
  ...
  perform set_config('request.jwt.claims',json_build_object('sub',v_roles->>'JEFE_TALLER','role','authenticated')::text,true);
  set local role authenticated;
  if public.registrar_salida_fisica(v_acta,'Vehiculo observado saliendo - prueba') is distinct from public.registrar_salida_fisica(v_acta,'Reintento de la prueba') then raise exception 'FALLO: salida duplicada'; end if;
  if (select count(*) from public.ot_salidas where entrega_id=v_acta)<>1 then raise exception 'FALLO: salida no registrada'; end if;
  reset role;
  perform set_config('request.jwt.claim.sub',v_roles->>'OPERARIO',true);
  perform set_config('request.jwt.claims',json_build_object('sub',v_roles->>'OPERARIO','role','authenticated')::text,true);
  set local role authenticated;
  v_negado:=false;
  begin perform public.registrar_salida_fisica(v_acta,'Intento de operario sin permiso');
  exception when insufficient_privilege then v_negado:=true; end;
  if not v_negado then raise exception 'FALLO: operario registra salida'; end if;
  reset role;
