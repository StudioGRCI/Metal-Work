...
begin
  ...
  for v_paso in 1..10 loop
    ...
    when 4 then
declare v_m uuid:=gen_random_uuid(); v_p uuid:=gen_random_uuid(); v_user record; n int;
begin
perform set_config('request.jwt.claim.sub',v_roles->>'JEFE_PRODUCCION',true);
set local role authenticated;
insert into ot_actividades(id,orden_id,area_id,nombre,peso_pct) select v_m,v_ot,id,'ENSAYO MTZ',100 from areas where codigo='MTZ';
insert into ot_actividades(id,orden_id,area_id,nombre,peso_pct) select v_p,v_ot,id,'ENSAYO PRD',100 from areas where codigo='PRD';
reset role;
for v_user in select u.id,r.codigo,a.codigo area from usuarios u join roles r on r.id=u.rol_id left join areas a on a.id=u.area_id where u.activo and r.codigo<>'ADMIN' loop
perform set_config('request.jwt.claim.sub',v_user.id::text,true);
perform set_config('request.jwt.claims',json_build_object('sub',v_user.id,'role','authenticated')::text,true);
set local role authenticated;
select count(*) into n from ot_actividades where id in(v_m,v_p);
if v_user.codigo in ('GERENTE','JEFE_PRODUCCION','JEFE_TALLER','DISENO') then
if n<>2 then raise exception 'FALLO: coordinador % no lee ambas',v_user.codigo; end if;
elsif v_user.codigo in ('VENDEDOR','ADMINISTRACION') then
if n<>0 then raise exception 'FALLO: oficina % lee hojas',v_user.codigo; end if;
else
if n<>(case when v_user.area in('MTZ','PRD') then 1 else 0 end) then raise exception 'FALLO: área % ve % hojas',v_user.area,n; end if;
end if;
reset role;
end loop;
perform set_config('request.jwt.claim.sub',v_roles->>'JEFE_PRODUCCION',true);
perform set_config('request.jwt.claims',json_build_object('sub',v_roles->>'JEFE_PRODUCCION','role','authenticated')::text,true);
set local role authenticated;
end;
